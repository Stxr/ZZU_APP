...
public class StringUtils {
    ...
    public static String getFileName(String content) {
        String result = null;//<img src ="/storage/emulated/0/zzu_app/PassagePic/1494242716156.txr"/>黑，你啊哈酒
        Pattern p_img = Pattern.compile(StaticConstant.BMOB_IAMGE_CACHE+"(.*?)/(.*?).txr");
        Matcher m_img = p_img.matcher(content);
        boolean result_img = m_img.find();
        if (result_img) {
            result = m_img.group(2);
        }
        return result;
    }
    /**
